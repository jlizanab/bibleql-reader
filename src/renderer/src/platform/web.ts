...
// Chromium, so this needs no Electron IPC and works unchanged inside any
// Chromium/WebKit-based webview (including a future Tauri shell). Don't
// reach for `dialog.showOpenDialog` here — it would add Electron coupling
...
// capability (Async Clipboard API) — not Electron-specific, so it needs
// no IPC and belongs here alongside pickImageFile, not in platform/electron.ts.
